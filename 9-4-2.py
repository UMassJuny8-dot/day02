# 설문조사 앱
# 버트, 체크박스, 라디오단추, 셀렉트박스, 멀티셀렉트, 슬라이더, 텍스트입력
# 위젯

import streamlit as st

st.title("🤦‍♂️미니 선호도 조사🤦‍♀️")
st.caption("위젯을 조작하면 화면 아래 '실시간 응답 요약'이 바로 바뀝니다.")

st.markdown("---")

# 1) 텍스트 입력 위젯 : key를 지정해서 다른 위젯과 이름이 겹치지 않게 한다.
name = st.text_input("1) 이름을 입력하세요.", value="황준영", key="widget_name")
# 2) 슬라이더 위젯 : 최소/최대/기본값 지정해 숫자로 선택하게 한다.
age = st.slider("2) 나이를 선택하세요.", min_value=10, max_value=80, value=24, key="widget_age")
# 3) 라디오단추 : 여러 선택지 중에서 하나만 고를 때 사용한다.
job = st.radio(
    "3) 직군을 선택하세요.", 
...
# 4) 셀렉트박스(드롭다운) : 라디오와 비슷하지만 목록이 길때 공간을 절약할수 있다.
country = st.selectbox(
    "4) 가장 관심있는 무역 상대국은 ???",
    options=['미국','중국','일본','영국','독일','스페인'],
    key="widget_country"
)

# 5) 멀티셀렉트 박스 : 여러개를 동시에 선택할수 있다.
interests = st.multiselect(
    "5) 관심있는 데이터 분야 모두 고르세요.",
...
# 7) 슬라이더 만족 점수
score = st.slider("7) 강의 만족도", min_value=1, max_value=5, value=3, key="widget_score")

# 8) 텍스트 영역 : 여러줄의 입력이 필요할때, 자유의견 입력
feedback = st.text_area("의견을 자유롭게 남겨주세요.", key="widget_feedback")
...
# 위젯 값들을 버튿을 누루지 않아도 조작하는 즉시 바로 갱신된다.
# 이름: 홍길동 / 나이:25살
